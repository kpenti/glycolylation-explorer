...
include("/www/htdocs/3fx.us/bio/libs/pChart2.1.3/class/pImage.class.php");$main_residue = (int) $_GET['residue'];
...
$file_save_path = "/www/htdocs/3fx.us/bio/gly/images/graphs_bw/$main_residue-$gly_residue.png";
if(file_exists($file_save_path) && $size!=1){
 ...
 $str = 'http://3fx.us/bio/gly/ajax/get_glycosylation_sites.php?residue='.$gly_residue;
...
#$glycosylation_data->addPoints($glycosylation_sites,"Glycosylation");
 $str = 'http://3fx.us/bio/gly/ajax/get_fixations.php?residue='.$main_residue;
...
foreach($fixations['data_points'] as $k => $v) {
	...
	#$MyData->setPalette($k,$serieSettings);
 ...
 foreach(array_keys($fixations['data_points']) as $k => $v) {
	$MyData->setSerieTicks($v,$k);
	$MyData->setSerieWeight($v,0);
		$MyData->setSerieShape($v, $shapes[$k]);

	$serieSettings = array("R"=>0,"G"=>0,"B"=>0);
	$MyData->setPalette($v, $serieSettings);
}
...
$height = 165;
 ...
 #$myPicture->drawPlotChart(array("PlotBorder"=>TRUE,"PlotSize"=>3,"BorderSize"=>1,"Surrounding"=>-60,"BorderAlpha"=>80));
  ...
  #$myPicture->drawLegend(945,10,array("Mode"=>LEGEND_VERTICAL));
